...
module.exports = {
  ...
  _getColSpecs: function (cols, opts) {
    ...
    return cols[0].map(function (val, idx) {
      return self._tblGrid(opts, val);
    });
  },

  // TODO
  _tblGrid: function (opts, val) {
    if (val.opts && val.opts.cellColWidth) {
      return {
        "w:gridCol": {
          "@w:w": val.opts.cellColWidth
        }
      };
    } else {
      return {
  ...
  _getCell: function (val, opts, tblOpts) {
    ...
    if (typeof val != 'string' && typeof val != 'number') {
      ...
      if (opts.language || tblOpts.language) {
        content[0]["w:p"]["w:r"]["w:rPr"]["w:lang"] = {
          "@w:val": opts.language || tblOpts.language
        }
      }
    }
    var cellObj = {
  ...
  _getComplexCell: function (cellVal, tblOpts) {
    ...
    var content = cellVal.map(function (cell) {
      ...
      if (cell.type === 'text' || cell.type === 'number') {
        ...
        if (opts.language || tblOpts.language) {
          rowObject[rowObject.length - 1]["w:r"]["w:rPr"]["w:lang"] = {
            "@w:val": opts.language || tblOpts.language
          }
        }
        return [
